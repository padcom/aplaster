// ----------------------------------------------------------------------------
// file: ParserU.pas - a part of AplaSter system
// date: 2005-09-08
// auth: Matthias Hryniszak
// desc: pascal script importer parser unit
// ----------------------------------------------------------------------------
...
//    property PrevToken: string read fPrevToken.data;
    ...
    procedure NextToken;
    procedure NextTokenSkipCurrent;
...
procedure TUnitParser.NextTokenSkipCurrent;
begin
  RemoveFromTokenHistory(fToken);
//  fPrevToken := fToken;
//  fprevOrgToken := fparser.OriginalToken;
  fParser.next;
  fToken.ID := fParser.CurrTokenID;
...
//                    RaiseError('overload directive does not applies to function/method pointers', TokenRow, TokenCol);
    ...
    // get the next token
        ...
        begin
          FinishedProcDecl := true;
          if Length(decl) > 255 then
            decl := Copy(decl, 1, 255) + '''+''' + Copy(decl, 256, MaxInt);
        end;
      end;
...
begin
  ...
  case aTokenID of
    ...
    CSTI_Identifier: // simple type by name  (MyInt = Integer)
      begin
        if (aTokenID = CSTI_Identifier) and (UpperCase(OrgToken) = 'PACKED') then
        begin
          NextTokenSkipCurrent;
